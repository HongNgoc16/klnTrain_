    ...
    <div
      className={`relative h-11 overflow-hidden rounded-lg border transition-colors ${
        disabled
          ...
          : 'border-amber-200 bg-white hover:border-[#FFD15A] focus-within:border-[#FFD15A]'
      }`}
    >
      <button
        type="button"
      ...
      <div className="absolute inset-0 flex items-center pl-9 pr-3 pointer-events-none select-none">
        {display
          ? <span className={`text-sm ${disabled ? 'text-gray-400' : 'text-neutral-800'}`}>{display}</span>
...
const Search = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    navigate('/tim-ve', {
      state: { fromStation, toStation, departureDate, returnDate, tripType, adultTickets, childTickets, ticketTotal },
    })
  }

  return (
    <form
      className="w-full max-w-sm rounded-md bg-[#FDF2D6]/90 p-5 shadow-xl sm:max-w-[420px]"
    ...
    >
      ...
      <div className="relative flex flex-col gap-3 sm:grid sm:grid-cols-[1fr_auto_1fr] sm:items-center sm:gap-2">
        <div className="relative">
          ...
          >
            ...
            {fromOptions.map((s) => (
              <option key={s.id_ga} value={s.ten_ga}>{s.ten_ga}</option>
            ))}
          </select>
        </div>
      ...
      <div className="mt-3" ref={ticketBoxRef}>
        ...
        {/* Collapsible ticket box */}
        <div className={`overflow-hidden transition-all duration-200 ease-in-out ${showTicketBox ? 'max-h-40 opacity-100 mt-1' : 'max-h-0 opacity-0'}`}>
          <div className="rounded-lg border border-amber-200 bg-white px-4 py-3 shadow-lg">
            <p className="mb-3 text-xs text-gray-400">Tối đa {MAX_TICKETS} vé / lần đặt</p>

            {[
              { key: 'adult', label: 'Người lớn', sub: 'Từ 10 tuổi', count: adultTickets, min: 1 },
              { key: 'child', label: 'Trẻ em',    sub: '6–9 tuổi · -25%', count: childTickets, min: 0 },
            ].map(({ key, label, sub, count, min }) => (
              <div key={key} className="flex items-center justify-between py-1.5 first:pb-2 last:pt-2 [&:not(:last-child)]:border-b border-gray-100">
                <div>
                ...
                <div className="flex items-center gap-2.5">
                  <button
                    ...
                    onClick={() => changeTicketCount(key, -1)}
                    disabled={count <= min}
                  >
                  ...
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded-full bg-[#8C1D19] text-white disabled:bg-gray-200 disabled:text-gray-400 transition-colors"
                    onClick={() => changeTicketCount(key, 1)}
                    disabled={ticketTotal >= MAX_TICKETS}
                  >
