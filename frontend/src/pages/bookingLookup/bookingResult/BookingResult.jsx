...
const BookingResult = ({ data, error, isLoading, onBack }) => {
  ...
  const handlePrint = () => {
    ...
    const tickets = veList.map(v => ({
      ...
      bookingCode,
      passenger:   { fullName: v.fullName, type: v.loaiHanhKhach === 'tre_em' ? 'child' : 'adult' },
      fromStation: v.gaDi,
      toStation:   v.gaDen,
      trainCode:   v.maTau,
              ...
              {tickets.map((v, i) => (
                <tr key={i} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2.5">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${v.loaiHanhKhach === 'tre_em' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>
                      {v.loaiHanhKhach === 'tre_em' ? 'Trẻ em' : 'Người lớn'}
                    </span>
                  </td>
