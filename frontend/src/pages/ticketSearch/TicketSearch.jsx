...
const TicketSearch = () => {
  ...
  // legAdults/legChildren được TrainSelection trả về qua onComplete
  const handleDepartureDone = (train, passengerSeats, totalPrice, sessionId, hetHan, legAdults, legChildren) => {
    const tripData = buildTripData(train, passengerSeats, totalPrice, fromStation, toStation, departureDate)
    if (isRoundTrip) {
      // Lưu thông tin chiều đi (chưa hold ghế)
      setDepartureInfo({ ...tripData, sessionId: null, legAdults, legChildren })
      setHoldError(null)
      setStep('return')
    } else {
      // Vé 1 chiều: sessionId + hetHan có giá trị
      navigate('/checkout', {
        state: {
          trips: [{ ...tripData, sessionId }],
          totalPassengers: legAdults + legChildren,
  ...
  // Chiều về: TrainSelection đã hold cả 2 chuyến → sessionId/hetHan đã hợp lệ
  const handleReturnDone = (train, passengerSeats, totalPrice, sessionId, hetHan, legAdults, legChildren) => {
    ...
    navigate('/checkout', {
      state: {
        trips: [
          { ...departureInfo, sessionId },   // sessionId từ hold chiều về (bao gồm cả chiều đi)
          { ...returnTripData, sessionId },
        ],
        totalPassengers: totalA + totalC,
        adultTickets:    totalA,
        childTickets:    totalC,
        tripType: 'round-trip',
        hetHan,
