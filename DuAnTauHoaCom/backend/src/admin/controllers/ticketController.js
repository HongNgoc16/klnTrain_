...
const { cancelTickets } = require('../../services/CancelService');
...
exports.cancelTicket = async (req, res) => {
  ...
  try {
    ...
    if (ticket.trang_thai !== 'da_xac_nhan' && ticket.trang_thai !== 'hieu_luc') {
      return res.status(400).json({ success: false, message: 'Vé không thể hủy ở trạng thái hiện tại' });
    }
    if (!['da_thanh_toan', 'da_xac_nhan'].includes(ticket.trang_thai_don)) {
      return res.status(400).json({ success: false, message: 'Đơn đặt vé chưa thanh toán, không thể hủy' });
    }

    // Hủy vé + tính phí hủy theo chính sách + tạo bản ghi hoàn tiền
    const result = await cancelTickets(ticket.ma_dat_cho, [parseInt(id)], ly_do || 'Admin hủy vé');
    ...
    res.status(status).json({ success: false, message: error.message || 'Lỗi hủy vé' });
  }
};
...
exports.confirmTicket = async (req, res) => {
  ...
  try {
    const checkResult = await executeQuery(`
      SELECT trang_thai FROM Ve WHERE id_ve = @id
    `, { id });
    ...
    if (ticket.trang_thai !== 'hieu_luc' && ticket.trang_thai !== 'da_xac_nhan') {
      return res.status(400).json({ success: false, message: 'Vé không ở trạng thái có hiệu lực để xác nhận' });
    }
    ...
    await executeQuery(`
      INSERT INTO AuditLog (bang, ma_ban_ghi, hanh_dong, gia_tri_moi, thoi_gian)
      VALUES ('Ve', @id, 'UPDATE', @log, GETDATE())
    `, { id, log: JSON.stringify({ trang_thai: 'da_su_dung' }) });
