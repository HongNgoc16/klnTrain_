...
exports.createCustomerDiscount = async (req, res) => {
  const { ten_chinh_sach, loai_hanh_khach, phan_tram_giam, tu_ngay, den_ngay } = req.body;

  if (!ten_chinh_sach || !loai_hanh_khach || phan_tram_giam === undefined) {
    return res.status(400).json({ success: false, message: 'Thiếu thông tin chính sách' });
  }

  try {
    const result = await executeQuery(
      `INSERT INTO ChinhSachGia (ten_chinh_sach, loai_hanh_khach, phan_tram_giam, tu_ngay, den_ngay)
       OUTPUT INSERTED.*
       VALUES (@ten_chinh_sach, @loai_hanh_khach, @phan_tram_giam, @tu_ngay, @den_ngay)`,
      { ten_chinh_sach, loai_hanh_khach, phan_tram_giam, tu_ngay: tu_ngay || null, den_ngay: den_ngay || null }
    );
    res.json({ success: true, message: 'Thêm chính sách giá thành công', data: result.recordset[0] });
  } catch (error) {
    console.error('Lỗi thêm chính sách giá:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
// Thêm chính sách hủy mới
exports.createCancelFee = async (req, res) => {
  const { gio_truoc_gio_chay, phi_huy } = req.body;

  if (gio_truoc_gio_chay === undefined || phi_huy === undefined) {
    return res.status(400).json({ success: false, message: 'Thiếu thông tin chính sách hủy' });
  }

  try {
    const result = await executeQuery(
      `INSERT INTO ChinhSachHuy (gio_truoc_gio_chay, phi_huy)
       OUTPUT INSERTED.*
       VALUES (@gio_truoc_gio_chay, @phi_huy)`,
      { gio_truoc_gio_chay, phi_huy }
    );
    res.json({ success: true, message: 'Thêm chính sách hủy thành công', data: result.recordset[0] });
  } catch (error) {
    console.error('Lỗi thêm chính sách hủy:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
...
// Thêm biểu giá mới (dịp đặc biệt)
exports.createOccasionPolicy = async (req, res) => {
  const { ten_dip, he_so_tang, ngay_bat_dau, ngay_ket_thuc, don_gia_km_goc } = req.body;

  if (!ten_dip || he_so_tang === undefined || !ngay_bat_dau || !ngay_ket_thuc) {
    return res.status(400).json({ success: false, message: 'Thiếu thông tin biểu giá' });
  }

  try {
    const result = await executeQuery(
      `INSERT INTO BieuGia (ten_dip, ngay_bat_dau, ngay_ket_thuc, he_so_tang, don_gia_km_goc, trang_thai)
       OUTPUT INSERTED.*
       VALUES (@ten_dip, @ngay_bat_dau, @ngay_ket_thuc, @he_so_tang, @don_gia_km_goc, 'dang_ap_dung')`,
      { ten_dip, ngay_bat_dau, ngay_ket_thuc, he_so_tang, don_gia_km_goc: don_gia_km_goc || 540 }
    );
    res.json({ success: true, message: 'Thêm biểu giá thành công', data: result.recordset[0] });
  } catch (error) {
    console.error('Lỗi thêm biểu giá:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};
