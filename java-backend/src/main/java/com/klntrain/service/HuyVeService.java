...
public class HuyVeService {
    ...
    public Map<String, Object> tinhPhiHuy(Long idVe) {
        Ve ve = veRepo.findById(idVe)
            .orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy vé"));

        if ("da_huy".equals(ve.getTrangThai())) {
            throw NgoaiLeUngDung.loiYeuCau("Vé đã bị hủy trước đó");
        }
    ...
    @Transactional
    public Map<String, Object> huyVe(String maDatCho, List<Long> idVeList, String lyDo) {
        ...
        for (Long idVe : idVeList) {
            Ve ve = veRepo.findById(idVe)
                .orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy vé " + idVe));
