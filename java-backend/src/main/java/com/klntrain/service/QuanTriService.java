...
public class QuanTriService {
    ...
    public Map<String, Object> layThongKeTongQuan() {
        long tongKhachHang = taiKhoanRepo.findAll().stream()
            .filter(tk -> "khach_hang".equals(tk.getVaiTro())).count();
        long tongTau = tauRepo.findByTrangThai("hoat_dong").size();
        long tongVe = veRepo.count();

        BigDecimal tongDoanhThu = donDatVeRepo.findAll().stream()
        ...
        return Map.of(
            ...
            "total_tickets", tongVe,
            "total_customers", tongKhachHang,
            "total_trains", tongTau
        );
    }

    // ─── Ga tàu ──────────────────────────────────────────────────
    public List<GaTau> layTatCaGaTau() { return gaTauRepo.findAll(); }

    public GaTau luuGaTau(GaTau ga) { return gaTauRepo.save(ga); }

    public void xoaGaTau(Long id) {
        if (!gaTauRepo.existsById(id)) throw NgoaiLeUngDung.khongTimThay("Không tìm thấy ga");
        gaTauRepo.deleteById(id);
    }

    // ─── Tàu ─────────────────────────────────────────────────────
    public List<Tau> layTatCaTau() { return tauRepo.findAllByOrderBySoHieuAsc(); }

    public Tau luuTau(Tau tau) { return tauRepo.save(tau); }

    public void xoaTau(Long id) {
        tauRepo.findById(id).orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy tàu"));
        tauRepo.deleteById(id);
    }

    // ─── Người dùng ──────────────────────────────────────────────
    public List<TaiKhoan> layTatCaNguoiDung() { return taiKhoanRepo.findAll(); }

    @Transactional
    public TaiKhoan capNhatTrangThaiNguoiDung(Long id, String trangThai) {
        TaiKhoan tk = taiKhoanRepo.findById(id)
            .orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy người dùng"));
        tk.setTrangThai(trangThai);
        return taiKhoanRepo.save(tk);
    }

    @Transactional
    public TaiKhoan capNhatVaiTroNguoiDung(Long id, String vaiTro) {
        TaiKhoan tk = taiKhoanRepo.findById(id)
            .orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy người dùng"));
        tk.setVaiTro(vaiTro);
        return taiKhoanRepo.save(tk);
    ...
    // ─── Khuyến mãi ──────────────────────────────────────────────
    public List<KhuyenMai> layTatCaKhuyenMai() { return khuyenMaiRepo.findAll(); }

    public KhuyenMai luuKhuyenMai(KhuyenMai km) { return khuyenMaiRepo.save(km); }

    public void xoaKhuyenMai(Long id) {
        if (!khuyenMaiRepo.existsById(id)) throw NgoaiLeUngDung.khongTimThay("Không tìm thấy mã khuyến mãi");
        khuyenMaiRepo.deleteById(id);
    }

    // ─── Lịch chạy ───────────────────────────────────────────────
    public List<LichChay> layTatCaLichChay(Long idTau) {
        return idTau != null
    ...
    @Transactional
    public LichChay taoLichChay(LichChay lichChay) { return lichChayRepo.save(lichChay); }

    @Transactional
    public void xoaLichChay(Long id) {
        LichChay lc = lichChayRepo.findById(id)
            .orElseThrow(() -> NgoaiLeUngDung.khongTimThay("Không tìm thấy lịch chạy"));
        if (chuyenTauRepo.countByIdLichChay(id) > 0)
            throw NgoaiLeUngDung.loiYeuCau("Không thể xóa lịch trình đã có chuyến tàu");
        lichChayRepo.delete(lc);
    }

    // ─── Thông báo ───────────────────────────────────────────────
    @Transactional
    public int phatSongThongBao(String tieuDe, String noiDung, String loai) {
    ...
    // ─── Đơn đặt vé ──────────────────────────────────────────────
    public List<DonDatVe> layTatCaDonDatVe(String trangThai) {
        if (trangThai != null) {
    ...
    public Optional<DonDatVe> layChiTietDon(Long id) {
        return donDatVeRepo.findById(id);
    }

    // ─── Thống kê biểu đồ ────────────────────────────────────────
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layDoanhThuTheoThang() {
        String sql = """
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layDoanhThuTheoTuan() {
        String sql = """
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layTuyenPhoBien() {
        String sql = """
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layDonGanDay() {
        String sql = """
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layChuyenSapKhoiHanh() {
        String sql = """
    ...
    @SuppressWarnings("unchecked")
    public List<Map<String, Object>> layGaTauHangDau() {
        String sql = """
