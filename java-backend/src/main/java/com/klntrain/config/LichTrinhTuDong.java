...
public class LichTrinhTuDong {
    ...
    public void giaiPhongGheHetHan() {
        try {
            int soGhe = em.createQuery(
                "UPDATE TamGiuGhe t SET t.trangThai = 'da_giai_phong' WHERE t.thoiGianHetHan < CURRENT_TIMESTAMP AND t.trangThai = 'dang_giu'"
            ).executeUpdate();
            if (soGhe > 0) log.info("[LichTrinh] Đã giải phóng {} ghế hết hạn", soGhe);
        } catch (Exception e) {
            log.warn("[LichTrinh] Lỗi giải phóng ghế: {}", e.getMessage());
        }
    }
    ...
    public void huyDonHetHan() {
        try {
            ...
            log.warn("[LichTrinh] Lỗi hủy đơn hết hạn: {}", e.getMessage());
        }
    }
}
