...
// Tạo mã vé 9 chữ số xác định từ seed (ổn định qua các lần render)
...
const TicketCard = ({ passenger, passengerIdx, trip, tripLabel, bookingCode, tripIdx }) => {
  ...
  const ticketCode = genTicketCode(`${bookingCode}${passengerIdx}${tripIdx ?? 0}`)
  ...
  const handlePrint = () => {
    const tickets = passengers.flatMap((passenger, pIdx) =>
      trips.map((trip, tIdx) => {
        ...
        const ticketCode = genTicketCode(`${bookingCode}${pIdx}${tIdx}`)
        return {
          ticketCode,
          bookingCode,
          passenger,
